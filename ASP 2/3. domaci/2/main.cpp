...
constexpr double EPSILON = 0.000000000000000001, RATIO = 0.99, AVG = 150;
constexpr int TABLE_SIZE = 3, MAX_SIZE = 1000000;
...
class HashTable {
    ...
    static vector<int> vPrimes;
    static int primesGenerated;
    int size, n = 0, cnt = 0, cntSucc = 0, unsuccAttempt = 0;
    Element* x;
...
public:
    ...
    HashTable(int _size) : size(_size), x(new Element[_size]), hashFunction(new HashDivision(_size)) {
        int c1 = 0, c2 = -1;
    ...
    enum Error { DIV_ZERO, NO_MEMORY };
};

vector<int> HashTable::vPrimes;
...
void HashTable::AdaptHashTable() {

    while ((fillRatio() > RATIO) || (avgAccessSuccess() > AVG) || (avgAccessUnsuccess() > AVG) ) {
        auto newSize = upper_bound(vPrimes.begin(), vPrimes.end(), size * 2);
        stack <int> sKeys;
        stack <string> sInfo;
        //cout << fillRatio() << " " << avgAccessSuccess() << " " << avgAccessUnsuccess() << " " << *newSize << endl;
        if (newSize == vPrimes.end())
            throw NO_MEMORY;

        for (int i = 0; i < size; i++)
            if (!x[i].deleted && !x[i].empty) {
                sInfo.push(*x[i].k.GetInfoPt());
                sKeys.push(x[i].k.GetKey());
            }

        size = *newSize;
        delete [] x; x = new Element[size];
        delete hashFunction; hashFunction = new HashDivision(size);

        cnt = 0; unsuccAttempt = 0;
        cntSucc = 0;

        while (!sKeys.empty()) {
            insertKey(sKeys.top(), sInfo.top());
            sKeys.pop(); sInfo.pop();
        }
    }
}
...
double HashTable::avgAccessSuccess() const {
    ...
    return static_cast<double>(cntSucc) / n;
}
...
void HashTable::GeneratePrimes(int n) {
...
int main() {
    int choice;
    HashTable::GeneratePrimes(MAX_SIZE);
    ...
    while (choice != 0) {
        ...
        try {
            ...
            else if (e == HashTable::NO_MEMORY)
                cout << "Preterao si ga" << endl;
        }
        OutputMenu(choice);
    }
